...
@st.cache_resource
def load_all_models():
    with open("best_model_compressed.pkl", "rb") as f_model, \
         open("scaler.pkl", "rb") as f_scaler, \
         open("encoder.pkl", "rb") as f_encoder:
        return {
            "model": pk.load(f_model),
            "scaler": pk.load(f_scaler),
            "encoder": pk.load(f_encoder)
        }
...
def run_prediction(input_df):
    ...
    if prediction == "Not_Canceled":
        st.success(f"✅ Booking is likely to be **{prediction}**")
    else:
        st.error(f"⚠️ Booking is likely to be **{prediction}**")
...
def booking_app():
    ...
    with col1:
        ...
        st.markdown("""
        - Adults: 2, Children: 0
        - Weekend Nights: 1, Week Nights: 2
        - Meal Plan: Meal Plan 1, Parking: Yes
        - Room Type: Room A, Lead Time: 30
        - Arrival: 2025-5-22, Market: Online
        - Repeated Guest: Yes
        - Prev Cancel: 0, Prev Not Cancel: 3
        - Price: 120.0, Special Requests: 1
        """)
        ...
        if st.button("Run Test Case 1"):
            ...
            with st.spinner("Testing case 1..."):
                run_prediction(test_input_1)
    ...
    with col2:
        ...
        st.markdown("""
        - Adults: 1, Children: 1
        - Weekend Nights: 1, Week Nights: 0
        - Meal Plan: Not Selected, Parking: No
        - Room Type: Room D, Lead Time: 2
        - Arrival: 2025-4-24, Market: Offline
        - Repeated Guest: No
        - Prev Cancel: 3, Prev Not Cancel: 0
        - Price: 200.0, Special Requests: 0
        """)
        ...
        if st.button("Run Test Case 2"):
            ...
            with st.spinner("Testing case 2..."):
                run_prediction(test_input_2)
